...
class Torrenttelik(AceProxyPlugin):
    ...
    def handle(self, connection, headers_only=False):
        ...
        if path.startswith('/torrent-telik/channel/'):
            name = path.rsplit('.', 1)
            if not name[1]:
            ...
            path = '' if len(self.channels) == 0 else '/torrent-telik/channel'
        ...
        if play: connection.handleRequest(headers_only, name, picons.logomap.get(name, None), fmt=params.get('fmt', [''])[0])
